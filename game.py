...
#be easily stored in the Game class.
BLACK = (0, 0, 0,)
RED = (255, 0, 0)
GREEN = (0, 255, 0)
...
class Game(object):
    ...
    #Function will be called in each "frame"; moves snake's head in the current
    #direction and (if we haven't eaten a fruit) removes the last segment of 
    #the snake.
    def move_snake(self):
    ...
    #Main functionality is trivial: check if any part of the snake has the same
    #position as the head; if it does, it means you're dead.
    def check_snake_hit(self):
        headless_snake = []
        for pos in self.snake:
    ...
    #all the things that check if we have already died in one place (especially
    #since we might add more death conditions in the future).
    ...
    #same position as the fruit. If yes, increments the score by 10 and returns
    #True, so that self.move_snake() knows not to remove the last item of the
    #self.snake.
    ...
    def loop(self):
        while self.gamestate==1:
            ...
            #an old guy, so I went with a pretty slow game, 40 gives 20-25 FPS.
